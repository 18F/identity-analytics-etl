...
def lambda_handler(event, context):
    ...
    This handler uses the master-user rotation scheme to rotate an Redshift user credential. During the first rotation, this
    ...
    {
        'endpoint': <required: redshift endpoint>,
...
def set_secret(service_client, arn, token):
    ...
    if current_dict['endpoint'] != master_dict['endpoint']:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT 1 FROM pg_group, pg_user WHERE pg_user.usesysid = ANY(pg_group.grolist) and pg_user.usename = %s", (pending_dict['username'],))
            if len(cur.fetchall()) == 0:
                create_user = "CREATE USER %s" % pending_dict['username']
                cur.execute(create_user + " PASSWORD %s", (pending_dict['password'],))
                cur.execute("GRANT %s TO %s IN GROUP %s" % (current_dict['username'], pending_dict['username'], current_dict['group']))
            else:
                alter_user = "ALTER USER %s" % pending_dict['username']
...
def get_connection(secret_dict):
    ...
    try:
        conn = psycopg2.connect(host=secret_dict['endpoint'], user=secret_dict['username'], password=secret_dict['password'], database=dbname, port=port, connect_timeout=5)
        return conn
...
def get_secret_dict(service_client, arn, stage, token=None):
    ...
    """
    required_fields = ['endpoint', 'username', 'password', 'dbname']
